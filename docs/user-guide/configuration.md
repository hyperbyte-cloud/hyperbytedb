...
HTTP rate limiting for `/write` and `/query`. See **[Rate limiting](rate-limiting.md)** for behavior, tuning, and client guidance.
...
Rejected requests increment `hyperbytedb_rate_limit_denied_total` (documented in [Administration](administration.md#monitoring)).
